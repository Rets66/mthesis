...
% しかし，転送量を少なくしたりレコードタイプを一般的に使用されるといった転送効率よりも秘匿性を優先させることで，既存の検知アプローチを迂回できる脅威がある．
 ...
 しかし，転送データ量の削減やパケット間のインターンバルを長期化させるといった転送効率を下げる手法を用いることで，既存の検知アプローチを迂回される脅威がある．
 ...
 評価では，実装した提案シスむのプロトタイプ上でトンネリング通信をシミュレーションさせることによって，DNSトンネリングの通信抑止に有効であることを示した．
...
\eabstract{
 ...
 The method called DNS Tunneling is the most popular in those covert network techniques.
 DNS tunneling is working with a rather simple way, it can transfer data inputting as Qname label in the DNS query packet.
 Against this tunneling technique, many countermesures have been proposed.
 Those previous detections work by using some features such as subdomain's length or entropy, and frequency of traffic by analysing tunneling techniques.
 ...
 My simulation of DNS tunneling in implemented prototype system shows it is effective for deterrence of the tunneling.
 Besides, the results I evaluated the features of the proposed system shows my proposed system is expected for a reduction of traffic when name resolution are requested and faster resoluting the name.
