...
% しかし，転送量を少なくしたりレコードタイプを一般的に使用されるといった転送効率よりも秘匿性を優先させることで，既存の検知アプローチを迂回できる脅威がある．
 巧妙化するサイバー攻撃では，通信を正規の通信に偽装させることによって，秘匿的に目的が実行される．
 DNSトンネリングは，このような秘匿通信手法の中で最も代表的な手法である．
 従来のDNSトンネリングへの対処には，サブドメインの長さやリソースレコードのタイプなどの特徴量に基づいて推定された閾値やモデルを用いた検知アプローチが取られてきた．
...
\eabstract{
 ...
 The previous countermeasure were based on the detection apploach estimated threshold or malicious model.
 Those apploachies are based on the some features such as the lenght of subdomain or the type of resouce records.
 ...
 Besides, the results I evaluated the features of the proposed system shows my proposed system is expected for a reduction of traffic when name resolution are requested and more fast name resolution.
}
