\begin{algorithm}[!h]
 ...
 \Fn{\Handler{query\_data}}{
	 $qname,\ qtype \leftarrow parser(query\_data)$\;
	 $record\_value \leftarrow db\_accesser(qname)$\;
	 \If{$value$}{
		 $payload \leftarrow benigh\_response(qname,\ qtype,\ ttl,\ record\_value) $\;
		}
		\Else{
		 $payload \leftarrow error\_response(qname,\ qtype)$\;
		 }
		$payload \leftarrow payload.pack()$\;
		$connection.sendto(payload,\ client\_address)$\;
 }
